...
public class OKXSocketClientUnifiedApiExchangeData : IOKXSocketClientUnifiedApiExchangeData
{
    ...
    /// <inheritdoc />
    public virtual async Task<CallResult<UpdateSubscription>> SubscribeToTickerUpdatesAsync(string symbol, Action<DataEvent<IEnumerable<OKXTicker>>> onData, CancellationToken ct = default)
    {
        return await _client.SubscribeInternalAsync(_client.GetUri("/ws/v5/public"),
            ...
            }, false, onData, ct).ConfigureAwait(false);
    }

    /// <inheritdoc />
    public virtual async Task<CallResult<UpdateSubscription>> SubscribeToOpenInterestUpdatesAsync(string symbol, Action<DataEvent<IEnumerable<OKXOpenInterest>>> onData, CancellationToken ct = default)
    {
        return await _client.SubscribeInternalAsync(_client.GetUri("/ws/v5/public"), 
            new List<Objects.Sockets.Models.OKXSocketArgs> 
            { 
