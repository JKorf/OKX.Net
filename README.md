...
[![Docs](https://img.shields.io/badge/Docs-OKX.Net-1b7f50?style=for-the-badge)](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=OKX.Net)
...
The [OKX.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=OKX.Net) is the main resource for installing, configuring, and using the library.

| Resource | Description |
|--|--|
| [Client guide](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=OKX.Net) | Installation, REST and WebSocket clients, authentication, dependency injection, error handling, and advanced features |
| [Examples](https://cryptoexchange.jkorf.dev/docs/exchange-clients/examples?library=OKX.Net) | Common REST and WebSocket operations |
| [API reference](https://cryptoexchange.jkorf.dev/docs/exchange-clients/reference?library=OKX.Net) | Client interfaces, methods, and properties |
...
For more examples and explanations, continue with the [OKX.Net documentation](https://cryptoexchange.jkorf.dev/docs/exchange-clients?library=OKX.Net) or browse the [compilable repository examples](https://github.com/JKorf/OKX.Net/tree/main/Examples).
...
V2 uses a strict interface per capability. The `SharedApi` property on each API surface therefore exposes only the operations or subscriptions that surface actually supports. `IOKXSharedApiClient` groups the exchange's Shared API surfaces for dependency injection and runtime capability lookup.
...
using var restClient = new OKXRestClient();
IGetTickerRest tickerClient = restClient.UnifiedApi.SharedApi;
...
The request and response models belong to `CryptoExchange.Net.SharedApis`, so the same operation can accept another exchange's `IGetTickerRest` implementation. When using dependency injection, inject `IOKXSharedApiClient` to access all of the exchange's Shared API surfaces or inject a capability such as `IGetTickerRest` directly. Use `GetCapability` on the aggregate when the operation, transport, or trading mode is selected at runtime.
...
## Release notes
* Version 5.5.0 - 24 Sep 2026
    * Updated CryptoExchange.Net to v13.0.0
    * Shared APIs
      * Added SharedApi V2 support
      * Added `.SharedApi` property to clients
      * Added `(I)OKXSharedApiClient` containing all V2 SharedApi references
      * Added `SharedApi` to `OKXOptions` to provide SharedApi related options
    ...
    * Added FeeGroups to OKXFeeRate response model
    * Added UnifiedApi.Account.GetNonTradableAssetsAsync endpoint
